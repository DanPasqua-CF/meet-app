...
defineFeature(feature, test => {
  ...
  test('User should be able to select a city from the suggested list', ({ given, and, when, then }) => {
    let AppComponent;
    let AppDOM; 
    let CitySearchDOM;
    let citySearchInput;
    given('user was typing in the City textbox', async () => {
      AppComponent = render(<App />);
      const user = userEvent.setup();
    ...
    and('the user should receive a list of upcoming events in the selected city.', async () => {
      const EventListDOM = AppDOM.querySelector('#event-list');
      const EventListItems = within(EventListDOM).queryAllByRole('listitem');
